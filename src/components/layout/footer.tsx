import Link from "next/link";
import { Mail, MapPin } from "lucide-react";

const links = {
  Network: [
    { label: "About Us", href: "/about" },
    { label: "Our Team", href: "/about/team" },
    { label: "Investment Thesis", href: "/about/thesis" },
    { label: "Portfolio", href: "/portfolio" },
    { label: "Events", href: "/events" },
    { label: "News", href: "/news" },
  ],
  Investors: [
    { label: "Why Join", href: "/investors" },
    { label: "How It Works", href: "/investors/how-it-works" },
    { label: "FAQs", href: "/investors/faq" },
    { label: "Apply to Join", href: "/apply/investor" },
  ],
  Startups: [
    { label: "Why Pitch to Us", href: "/startups" },
    { label: "Pitch Process", href: "/startups/how-it-works" },
    { label: "Opportunities", href: "/startups/opportunities" },
    { label: "Submit Pitch", href: "/apply/startup" },
  ],
};

export function Footer() {
  return (
    <footer className="bg-forest-dark text-cream">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        {/* Top */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-12 border-b border-white/10">
          {/* Brand */}
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-gold rounded-sm flex items-center justify-center flex-shrink-0">
                <span className="text-forest-dark font-bold text-sm">AAN</span>
              </div>
              <div>
                <p className="font-bold text-cream text-base leading-tight">
                  Abeokuta Angels Network
                </p>
                <p className="text-cream/60 text-xs">Abeokuta, Ogun State</p>
              </div>
            </div>
            <p className="text-cream/70 text-sm leading-relaxed max-w-xs mb-6">
              Organized belief, with a cheque book. We back overlooked builders
              solving real problems in Abeokuta and beyond.
            </p>
            <div className="flex items-center gap-2 text-cream/60 text-sm mb-2">
              <MapPin size={14} />
              <span>Abeokuta, Ogun State, Nigeria</span>
            </div>
            <div className="flex items-center gap-2 text-cream/60 text-sm mb-6">
              <Mail size={14} />
              <a
                href="mailto:hello@abeokutaangels.ng"
                className="hover:text-gold transition-colors"
              >
                hello@abeokutaangels.ng
              </a>
            </div>
            {/* Social */}
            <div className="flex items-center gap-3">
              {[
                { label: "X", href: "#", svg: <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z" /> },
                { label: "LinkedIn", href: "#", svg: <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /> },
                { label: "Instagram", href: "#", svg: <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" /> },
              ].map(({ label, href, svg }) => (
                <a
                  key={label}
                  href={href}
                  aria-label={label}
                  className="w-8 h-8 rounded-sm bg-white/10 flex items-center justify-center hover:bg-gold hover:text-forest-dark transition-all"
                >
                  <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-current">{svg}</svg>
                </a>
              ))}
            </div>
          </div>

          {/* Link columns */}
          {Object.entries(links).map(([group, items]) => (
            <div key={group}>
              <p className="text-gold text-xs font-semibold uppercase tracking-widest mb-4">
                {group}
              </p>
              <ul className="flex flex-col gap-2.5">
                {items.map((item) => (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className="text-cream/60 text-sm hover:text-cream transition-colors"
                    >
                      {item.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        {/* Bottom */}
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-cream/40 text-xs">
            © {new Date().getFullYear()} Abeokuta Angels Network. All rights reserved.
          </p>
          <div className="flex items-center gap-4">
            <Link
              href="/privacy-policy"
              className="text-cream/40 text-xs hover:text-cream/70 transition-colors"
            >
              Privacy Policy
            </Link>
            <Link
              href="/terms"
              className="text-cream/40 text-xs hover:text-cream/70 transition-colors"
            >
              Terms of Use
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
